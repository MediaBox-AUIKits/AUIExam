        ...
        switch (msgId) {
          ...
          case 107: // 拉流心跳
            reporter.subscribeStats({
              ...
              abps: args.abps,
              ajb: args.ajb,
              candiRemoteProtocol: args.cand_re_prot,
              candiRemoteAddr: args.cand_re_addr,
              candiLocalProtocol: args.cand_lo_prot,
              candiLocalAddr: args.cand_lo_addr,
            });
            break;
      ...
      switch (event.status) {
        ...
        case EConnectStatus.CONNECT_STATUS_DISCONNECTED:
          ...
          if (this._type === ERtsType.Subscribe) {
            ...
            // 1. GRTN UDP 包进不来，GRTN 认为响应超时主动断开
            // 2. GRTN 判定帧率不合格（<3fps持续20s），主动断开
